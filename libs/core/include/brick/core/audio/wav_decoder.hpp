...
class WavDecoder {
 public:
  static bool decode(interfaces::storage::IFileSystem& file_system,
                     const char* path, AudioBuffer& output) {
    ...
    while (true) {
      ...
      if (std::memcmp(chunk, "fmt ", 4) == 0) {
        ...
        if (fmt.size() < 16 || !read_exact(*file, fmt.data(), fmt.size())) return false;
        ...
        if (!file->seek(static_cast<long>(chunk_size), SEEK_CUR)) return false;
        position += static_cast<long>(chunk_size);
        have_data = true;
      } else {
        if (!file->seek(static_cast<long>(chunk_size), SEEK_CUR)) return false;
      ...
      if (chunk_size & 1U) {
        if (!file->seek(1, SEEK_CUR)) return false;
        ++position;
      }
      if (have_fmt && have_data) break;
    }

    if (!have_fmt || !have_data || format != 1 ||
    ...
    if (frame_count == 0 || !file->seek(data_offset, SEEK_SET)) return false;

    std::vector<std::uint8_t> samples(frame_count);
    std::vector<std::uint8_t> input(4096);
    std::size_t remaining = frame_count;
    std::size_t output_index = 0;
    while (remaining > 0) {
      const std::size_t frames = std::min(remaining, input.size() / bytes_per_frame);
      const std::size_t bytes = frames * bytes_per_frame;
      if (!read_exact(*file, input.data(), bytes)) return false;
      for (std::size_t i = 0; i < frames; ++i) {
        int mixed = 0;
        const auto* frame = input.data() + i * bytes_per_frame;
        if (bits == 8) {
          for (std::size_t channel = 0; channel < channels; ++channel) {
            mixed += static_cast<int>(frame[channel]) - 128;
          }
          mixed = mixed / static_cast<int>(channels) + 128;
        } else {
          for (std::size_t channel = 0; channel < channels; ++channel) {
            const auto* sample = frame + channel * 2;
            mixed += static_cast<std::int16_t>(read_u16(sample));
          }
          mixed = 128 + (mixed / static_cast<int>(channels) >> 8);
        }
        samples[output_index++] = static_cast<std::uint8_t>(
 ...
 private:
  static bool read_exact(interfaces::storage::IFile& file, void* data,
                         std::size_t size) {
  ...
  static std::uint32_t read_u32(const std::uint8_t* data) {
    return static_cast<std::uint32_t>(data[0] | (data[1] << 8) |
                                      (data[2] << 16) | (data[3] << 24));
  }
};
