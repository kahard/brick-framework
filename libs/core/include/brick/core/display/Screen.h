...
public:
    ...
    public:
        ...
        bool present() const
        {
            if (!valid())
                return false;
            const brick::interfaces::display::PixelBuffer view{ reinterpret_cast<const std::uint8_t*>(pixels_.get()),          static_cast<std::uint32_t>(area_.width),         static_cast<std::uint32_t>(area_.height),
                                                                static_cast<std::size_t>(area_.width) * sizeof(std::uint16_t), brick::interfaces::display::PixelFormat::rgb565, false };
    ...
    bool draw(brick::interfaces::display::DisplayRect area, const brick::interfaces::display::PixelBuffer& buffer)
    {
        if (area.empty() || area.width != static_cast<std::int32_t>(buffer.width) || area.height != static_cast<std::int32_t>(buffer.height)
            || !brick::interfaces::display::DisplayRect{ 0, 0, static_cast<std::int32_t>(device_.size().width), static_cast<std::int32_t>(device_.size().height) }.contains(area) || buffer.format != device_.pixel_format())
        {
            return false;
        }
    ...
    {
        const auto display_size = device_.size();
        return draw({ 0, 0, static_cast<std::int32_t>(display_size.width), static_cast<std::int32_t>(display_size.height) }, buffer);
    }

    bool fill(brick::interfaces::display::WritablePixelBuffer& buffer, std::uint16_t color) const
    {
    ...
    template <typename Glyph>
    bool draw_text(brick::interfaces::display::WritablePixelBuffer& buffer, std::int32_t x, std::int32_t y, const char* text, const char* characters, const Glyph* glyphs, std::size_t count, std::uint16_t color) const
    {
        if (buffer.data == nullptr || text == nullptr || characters == nullptr || glyphs == nullptr || buffer.format != brick::interfaces::display::PixelFormat::rgb565)
            return false;
        for (const char* cursor = text; *cursor != '\0'; ++cursor)
        {
            std::size_t index = 0;
            while (index < count && characters[index] != *cursor)
                ++index;
            if (index >= count)
                continue;
            ...
            x += glyph.width + 1;
            if (x >= static_cast<std::int32_t>(buffer.width))
                break;
        }
        return true;
    }

    bool wait_for_transfer_complete(std::uint32_t timeout_ms) { return device_.wait_for_transfer_complete(timeout_ms); }
