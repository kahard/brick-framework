...
class MipiDsiDisplay final : public brick::interfaces::display::IDisplayDevice {
 public:
  ...
  bool begin() override {
    ...
    if (esp_lcd_new_dsi_bus(&bus_config, &bus_) != ESP_OK) return false;

    esp_lcd_dbi_io_config_t io_config = {};
    io_config.virtual_channel = 0;
    io_config.lcd_cmd_bits = 8;
    io_config.lcd_param_bits = 8;
    if (esp_lcd_new_panel_io_dbi(bus_, &io_config, &io_) != ESP_OK) return false;
    ...
    if (esp_lcd_new_panel_dpi(bus_, &dpi_config, &panel_) != ESP_OK) return false;
 ...
 private:
  ...
  bool apply_rotation_() {
    ...
    return esp_lcd_panel_swap_xy(panel_, swap) == ESP_OK &&
           esp_lcd_panel_mirror(panel_, mirror_x, mirror_y) == ESP_OK;
  }
