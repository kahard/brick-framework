...
bool I2cEeprom::read(std::uint32_t address, void* data, std::size_t size)
{
    if (config_.i2c == nullptr || data == nullptr || address >= config_.capacity || size > config_.capacity - address)
        return false;
    return HAL_I2C_Mem_Read(config_.i2c, config_.device_address, static_cast<std::uint16_t>(address),
                            I2C_MEMADD_SIZE_16BIT, static_cast<std::uint8_t*>(data), size,
                            config_.timeout_ms) == HAL_OK;
}
...
bool I2cEeprom::write(std::uint32_t address, const void* data, std::size_t size)
{
    ...
    while (size != 0)
    {
        ...
        if (HAL_I2C_Mem_Write(config_.i2c, config_.device_address, static_cast<std::uint16_t>(address),
                              I2C_MEMADD_SIZE_16BIT, const_cast<std::uint8_t*>(bytes), chunk,
                              config_.timeout_ms) != HAL_OK)
            return false;
