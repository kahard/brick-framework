...
{
    return g_usb_host != nullptr && g_usb_host->storage_ready() ? 0 : STA_NOINIT;
}
...
bool Stm32UsbFile::seek(long offset, int origin)
{
    DWORD target = 0;
    if (origin == 0)
        target = static_cast<DWORD>(offset);
    else if (origin == 1)
        target = f_tell(&file_) + static_cast<DWORD>(offset);
    else if (origin == 2)
        target = f_size(&file_) + static_cast<DWORD>(offset);
    else
        return false;
...
std::unique_ptr<brick::interfaces::storage::IFile> Stm32UsbFileSystem::open(const char* path, const char* mode)
{
    ...
    if (f_open(&file, path, flags) != FR_OK)
        return nullptr;
    if (std::strchr(mode, 'a') != nullptr && f_lseek(&file, f_size(&file)) != FR_OK)
    {
        f_close(&file);
        return nullptr;
    }
